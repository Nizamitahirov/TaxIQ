...
export function CommandPalette({ open, onOpenChange, mode }: { open: boolean; onOpenChange: (o: boolean) => void; mode: 'all' | 'create' }) {
  ...
  const commands = useMemo<Cmd[]>(() => {
    const create: Cmd[] = CREATE_ACTIONS.filter((a) => isSuperAdmin || canAccess(a.module))
      .map((a) => ({ id: `c:${a.label}`, label: a.label, icon: a.icon, run: () => go(a.href), group: 'Yarat' }));
    if (mode === 'create') return create;
    const areas: Cmd[] = AREAS.map((a) => ({ id: `a:${a.key}`, label: a.label, hint: 'Bölmə', icon: a.icon, run: () => go(a.landing), group: 'Bölmələr' }));
    const nav: Cmd[] = navItems.map((i) => ({ id: `n:${i.href}`, label: t(i.labelKey), icon: i.icon, run: () => go(i.href), group: 'Naviqasiya' }));
    const fixed: Cmd[] = [
      { id: 'n:/launch', label: 'Bölmələr (Hub)', icon: LayoutGrid, run: () => go('/launch'), group: 'Naviqasiya' },
    ];
    return [...fixed, ...nav, ...areas, ...create];
  }, [mode, navItems, canAccess, isSuperAdmin, t]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 pt-[12vh]" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-popover shadow-2xl animate-in fade-in-0 zoom-in-95">
        <div className="flex items-center gap-3 border-b border-border px-4">
          ...
          <input ref={inputRef} value={q} onChange={(e) => setQ(e.target.value)} placeholder={mode === 'create' ? 'Nə yaratmaq istəyirsiniz?' : 'Səhifə, bölmə və ya əməliyyat axtar…'}
        ...
        <div className="flex items-center gap-4 border-t border-border px-4 py-2 text-[11px] text-muted-foreground">
          <span className="flex items-center gap-1"><kbd className="rounded border border-border px-1">↑</kbd><kbd className="rounded border border-border px-1">↓</kbd> naviqasiya</span>
          <span className="flex items-center gap-1"><kbd className="rounded border border-border px-1">↵</kbd> seç</span>
          <span className="ml-auto flex items-center gap-1"><kbd className="rounded border border-border px-1">⌘K</kbd> aç/bağla</span>
        </div>
      </div>
    </div>
  );
