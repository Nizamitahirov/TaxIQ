...
export function MobileTabbar() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex items-stretch border-t border-border bg-card/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-md lg:hidden">
      {item('/launch', 'Bölmələr', LayoutGrid, pathname === '/launch')}
      {item('/dashboard', 'Panel', LayoutDashboard, pathname === '/dashboard')}
      {areas.map((a) => item(a.landing, a.label.split(' ')[0], a.icon, activeArea === a.key, a.gradient))}
    </nav>
  );
}
