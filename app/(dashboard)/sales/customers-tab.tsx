      ...
      <div className="mb-4 flex items-center justify-between">
        <ExportButton filename="musteriler" rows={data ?? []}
          columns={[
            ...
            { header: 'Tip', value: (c) => c.type === 'legal_entity' ? 'Hüquqi' : 'Fərdi' },
            { header: 'Ödəniş müddəti', value: (c) => c.paymentTermDays ?? 0 },
          ]} />
        {canCreate && <Button size="sm" onClick={openNew}><Plus className="h-4 w-4" /> Yeni müştəri</Button>}
      </div>
      {isLoading ? <div className="flex justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : (data ?? []).length === 0 ? (
        <EmptyState title="Müştəri yoxdur" />
      ) : (
        <Card className="rounded-card"><CardContent className="overflow-x-auto p-0">
          <Table>
            <TableHeader><TableRow><TableHead>Ad</TableHead><TableHead>VÖEN</TableHead><TableHead>Tip</TableHead><TableHead>Ödəniş müddəti</TableHead><TableHead>Status</TableHead><TableHead></TableHead></TableRow></TableHeader>
            <TableBody>
              {(data ?? []).map((c) => (
                <TableRow key={c.id}>
                  <TableCell className="font-medium">{c.name}{c.isRelatedParty && <Badge variant="secondary" className="ml-2">əlaqəli</Badge>}</TableCell>
                  <TableCell className="text-muted-foreground">{c.taxId ?? '—'}</TableCell>
                  <TableCell>{c.type === 'legal_entity' ? 'Hüquqi şəxs' : 'Fərdi'}</TableCell>
                  <TableCell>{c.paymentTermDays ?? 0} gün</TableCell>
                  <TableCell><Badge variant={c.isActive ? 'success' : 'secondary'}>{c.isActive ? 'aktiv' : 'deaktiv'}</Badge></TableCell>
...
function CustomerDialog({ open, onOpenChange, companyId, actorUid, baseCurrency, edit, onSaved }: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>{edit ? 'Müştərini redaktə et' : 'Yeni müştəri'}</DialogTitle></DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2"><Label>Tip</Label>
              <Select value={type} onValueChange={(v) => setType(v as 'individual' | 'legal_entity')}>
                ...
                <SelectContent><SelectItem value="legal_entity">Hüquqi şəxs</SelectItem><SelectItem value="individual">Fərdi şəxs</SelectItem></SelectContent>
              </Select>
            </div>
            <div className="space-y-2"><Label>VÖEN</Label><Input value={taxId} onChange={(e) => setTaxId(e.target.value)} /></div>
          </div>
          <div className="space-y-2"><Label>Ödəniş müddəti (gün)</Label><Input type="number" value={term} onChange={(e) => setTerm(e.target.value)} /></div>
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={related} onChange={(e) => setRelated(e.target.checked)} /> Əlaqəli tərəf (IAS 24)</label>
        </div>
